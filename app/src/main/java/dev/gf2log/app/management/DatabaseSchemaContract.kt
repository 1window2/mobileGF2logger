...
internal data class DatabaseSchemaContract(
    ...
    companion object {
        ...
        private fun readColumns(
            ...
            table: String,
        ): Map<String, ColumnContract> {
            val extended = runCatching {
                database.rawQuery(
                    "PRAGMA table_xinfo(${identifier(table)})",
                    null,
                ).use { cursor ->
                    if (cursor.columnCount < 7) return@use null
                    readColumns(cursor, hiddenColumn = 6)
                }
            }.getOrNull()
            if (extended != null) return extended

            // Android API 26 ships SQLite 3.18, predating table_xinfo. The app
            // schema uses only ordinary columns, so table_info is equivalent.
            return database.rawQuery(
                "PRAGMA table_info(${identifier(table)})",
                null,
            ).use { cursor -> readColumns(cursor, hiddenColumn = null) }
        }

        private fun readColumns(
            cursor: android.database.Cursor,
            hiddenColumn: Int?,
        ): Map<String, ColumnContract> = cursor.run {
            buildMap {
                while (moveToNext()) {
                    put(
                        getString(1),
                        ColumnContract(
                            type = getString(2).uppercase(),
                            notNull = getInt(3) == 1,
                            defaultValue = getStringOrNull(4),
                            primaryKeyPosition = getInt(5),
                            hidden = hiddenColumn?.let(::getInt) ?: 0,
                        ),
                    )
                }
            }
